...
class Sound {
 public:
  Sound(const uint8_t bank, const uint8_t instrument, const uint8_t volume, const bool loop, const uint8_t len, const uint8_t* const notes,
        const uint8_t* const durations, const uint8_t transpose = 0, const uint8_t duration_offset = 0) :
      bank(bank), instrument(instrument), volume(volume), loop(loop), len(len), transpose(transpose), duration_offset(duration_offset),
